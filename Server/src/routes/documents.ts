...
// Wrapper to catch async errors and forward to express error middleware
const asyncHandler =
...
function multerErrorHandler(err: any, _req: Request, res: Response, next: NextFunction) {
  ...
  // cloudinary / storage errors often come as objects
...
// FILE ACCESS (redirect to Cloudinary URL)
documentsRouter.get(
  "/file/:id",
  requireAuth,
...
// UPLOAD (with JSON error handling keeps frontend clean)
documentsRouter.post(
  "/upload",
  requireAuth,
  requireRole(["candidate", "employer", "hr"]),
  // IMPORTANT: wrap multer so its errors are handled as JSON
  ...
  asyncHandler(async (req: MulterAuthedRequest, res) => {
    ...
    const doc = await Document.create({
      ...
      // legacy GridFS fields (ok to be undefined if model made optional)
      gridFsId: undefined,
      bucketName: "docs",
...
// -----------------------
// Router-level error handler (JSON only)
// IMPORTANT: mount AFTER routes in your server as well, but this keeps it safe if you `app.use(documentsRouter)`.
// -----------------------
